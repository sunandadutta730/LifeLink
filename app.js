...
function handleGoogleAuth() {
  if (typeof firebase === 'undefined' || !firebase.apps.length) {
    // Demo fallback if offline/mock environment
    const googleUser = {
      name: 'Google User',
    ...
    saveAccountToLocalStore(googleUser);
    saveUserAccountToFirebase(googleUser);
  ...
  firebase.auth().signInWithPopup(provider).then((result) => {
    ...
    const googleAccount = {
      ...
      email: user.email,
      phone: user.phoneNumber || '+91 98000 00000',
    ...
    // Demo fallback for popup blocking or unconfigured domains
    const googleUser = {
      name: 'Google Lifesaver',
      email: 'donor.google@gmail.com',
    ...
    setLoggedInUser(googleUser);
    closeModal();
    showToast(`🌐 Signed in with Google Account (${googleUser.email})!`, 'success');
    navigateTo('dashboard');
  });
}
